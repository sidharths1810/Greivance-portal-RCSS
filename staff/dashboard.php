...
if ($conn instanceof mysqli) {
    try {
        $sql = "SELECT  u.username, st.name, st.email, st.profile_image, st.staff_type
                FROM users u
...
if ($conn instanceof mysqli) {
    try {
        $sql = "SELECT  g.id, g.grievance_number, g.subject, g.description, g.attachment_path,
                        g.status, g.reply_details, g.created_at, g.updated_at, g.grievance_type_id,
                        gt.type_name
                FROM grievances g
...
<head>
  ...
  <meta name="description" content="Staff grievance dashboard — Rajagiri College Grievance Redressal Portal." />
...
<body class="min-h-screen bg-white text-slate-700 font-sans antialiased selection:bg-sun selection:text-ink">
    ...
    <div class="relative w-full max-w-2xl bg-white rounded-2xl border-2 border-ink shadow-[8px_8px_0_#FFC93C] animate-modal-in overflow-hidden flex flex-col max-h-[90vh]">
      ...
      <div class="p-6 overflow-y-auto flex-1 space-y-5">
        ...
        <div id="vgAttachmentWrapper" class="hidden">
          <p class="text-[10px] font-bold uppercase tracking-wider text-hot mb-1.5">Attachment</p>

          <div class="bg-blush rounded-xl p-4 border-2 border-ink space-y-3">
            <div class="flex flex-wrap items-center gap-2">
              <span class="inline-flex items-center gap-1.5 text-xs font-semibold text-ink">
                <i data-lucide="paperclip" class="w-3.5 h-3.5 text-hot"></i>
                <span id="vgAttachmentName" class="break-all">attachment</span>
              </span>
            </div>

            <div class="flex flex-wrap items-center gap-2">
              <button type="button" id="vgPreviewBtn" onclick="openPreviewOverlay()"
                      class="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-bold
                             bg-hot hover:bg-hotdark text-white border-2 border-ink
                             shadow-[2px_2px_0_#1F0A24] transition-transform hover:-translate-y-0.5 active:translate-y-0">
                <i data-lucide="eye" class="w-3.5 h-3.5"></i>
              ...
              <a id="vgDownloadLink" href="#" target="_blank" rel="noopener"
                 class="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-bold
                        bg-white hover:bg-blush text-ink
  ...
  <div id="previewOverlay" class="hidden fixed inset-0 z-[80] flex items-center justify-center p-4">
    ...
    <div class="relative w-full max-w-5xl max-h-[92vh] bg-ink border-2 border-ink rounded-2xl shadow-[8px_8px_0_#FFC93C] animate-modal-in overflow-hidden flex flex-col">
      <div class="flex items-center justify-between px-5 py-3 bg-ink border-b-2 border-white/10">
        <div class="flex items-center gap-2 min-w-0">
          <i data-lucide="image" class="w-4 h-4 text-sun flex-shrink-0"></i>
          <p id="previewFileName" class="text-sm font-semibold text-white truncate">Attachment</p>
        </div>
        ...
        <div class="flex items-center gap-2 flex-shrink-0">
          <a id="previewOpenNewTab" href="#" target="_blank" rel="noopener"
             class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold
                    bg-white/10 hover:bg-white/20 text-white border border-white/20 transition-colors">
            <i data-lucide="external-link" class="w-3.5 h-3.5"></i>
            <span>Open in new tab</span>
          </a>
          <button type="button" onclick="closePreviewOverlay()" aria-label="Close preview"
                  class="w-8 h-8 rounded-lg bg-white/10 hover:bg-hot
  ...
  <script>
    ...
    function openViewGrievanceModal(number, type, subject, description, status, reply, date, attachment) {
      ...
      if (attPath !== '' && attWrapper) {
        ...
        currentAttachmentPath = ''; currentAttachmentUrl = ''; currentAttachmentExt = ''; currentAttachmentName = '';
    ...
    function closeViewGrievanceModal() {
      if (previewOverlay && !previewOverlay.classList.contains('hidden')) closePreviewOverlay();
    ...
    document.addEventListener('keydown', function (e) {
      ...
      if (previewOverlay && !previewOverlay.classList.contains('hidden')) { closePreviewOverlay(); return; }
