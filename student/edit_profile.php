...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($formErrors) && $conn !== null) {
        try {
            ...
            $chk->close();
        } catch (Throwable $ex) { error_log('[Edit Profile Username Check] ' . $ex->getMessage()); }
    }
    ...
    if (empty($formErrors) && $conn !== null) {
        try {
            ...
            $chk->execute();
            if ($chk->get_result()->num_rows > 0) $formErrors[] = 'This email is already registered to another student.';
            $chk->close();
        } catch (Throwable $ex) { error_log('[Edit Profile Email Check] ' . $ex->getMessage()); }
    }
    ...
    if (empty($formErrors) && $conn !== null) {
        ...
        try {
            ...
            if ($newProfilePictureRelative !== null) {
                ...
                if (!$stmt) throw new Exception('Prepare failed (students update with image): ' . $conn->error);
                ...
                $stmt = $conn->prepare($sqlStud);
                if (!$stmt) throw new Exception('Prepare failed (students update): ' . $conn->error);

                $stmt->bind_param('sssssi', $name, $address, $email, $contactNumber, $whatsappNumber, $userId);
            }

            if (!$stmt->execute()) throw new Exception('Failed to update student profile: ' . $stmt->error);
            $stmt->close();
...
<head>
  ...
  <meta name="description" content="Edit student profile — Rajagiri College Grievance Redressal Portal." />
        ...
        <div class="flex items-center justify-between px-4 sm:px-6 py-3 md:py-4">
          ...
          <div class="flex items-center gap-3">
            <?php if ($existingPreviewUrl): ?>
              <img src="<?= e($existingPreviewUrl) ?>" alt="Student" class="w-10 h-10 rounded-full object-cover border-2 border-ink shadow-[2px_2px_0_#FFC93C]" />
            <?php else: ?>
            ...
            <?php endif; ?>
            <span class="hidden sm:block text-sm font-bold text-ink"><?= e($formData['username'] ?: 'Student') ?></span>
          </div>
        </div>
      </header>
