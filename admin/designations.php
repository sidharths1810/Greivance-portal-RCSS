...
<head>
  ...
  <meta name="description" content="Designation management — Rajagiri College Grievance Redressal Portal." />
...
<body class="min-h-screen bg-white text-slate-700 font-sans antialiased selection:bg-sun selection:text-ink">
      ...
      <main id="main-content" class="flex-1 px-4 sm:px-6 py-8">
        ...
        <div class="max-w-6xl mx-auto mb-6 rise">
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <h1 class="font-display text-2xl md:text-3xl font-extrabold tracking-tight text-ink mb-2">Designation</h1>
              <nav class="flex flex-wrap items-center gap-2 text-sm text-slate-500" aria-label="Breadcrumb">
                ...
                <span class="text-slate-300">/</span>
                <span class="text-hot font-semibold">Designation</span>
              </nav>
            </div>

            <button type="button" onclick="openDesignationModal('add')" title="Add Designation" aria-label="Add designation"
                      ...
                      <td colspan="5" class="px-6 py-16 text-center text-slate-500">
                        <div class="flex flex-col items-center justify-center">
                          <span class="w-16 h-16 rounded-full bg-blush border-2 border-ink flex items-center justify-center mb-4">
                            <i data-lucide="briefcase" class="w-8 h-8 text-hot"></i>
                          </span>
                          <p class="text-lg font-bold text-ink">No designations yet</p>
                          <p class="text-sm text-slate-500 mt-1 mb-4">Click "Add Designation" to create your first designation.</p>
                        </div>
                      </td>
                    </tr>
                  <?php else: ?>
                    <?php foreach ($designations as $index => $designation): ?>
                      ...
                      ?>
                      <tr class="designation-row hover:bg-blush/60 transition-colors group">
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-slate-900"><?= $index + 1 ?></td>
                        <td class="px-6 py-4 text-sm font-bold text-ink"><?= e($designationName) ?></td>
                        <td class="px-6 py-4 text-sm text-slate-600 max-w-[320px]"><?= e($designationDesc !== '' ? $designationDesc : '—') ?></td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                          <div class="flex items-center justify-center gap-1.5">
                            <button type="button" title="Edit designation" aria-label="Edit designation"
                            ...
                            <button type="button" title="View details" aria-label="View designation"
                            ...
                            <button type="button" title="Delete designation" aria-label="Delete designation"
                            ...
                            <button type="button" title="<?= $isActive ? 'Deactivate designation' : 'Activate designation' ?>" aria-label="Toggle status"
                                    onclick='confirmToggleStatus(<?= $designationId ?>, <?= json_encode($designationName) ?>, <?= json_encode($status) ?>)'
                                    class="w-9 h-9 rounded-full bg-blush border-2 border-ink flex items-center justify-center text-ink hover:bg-sun transition-all duration-200">
            ...
            <?php if (!empty($designations)): ?>
              <div class="px-6 py-4 bg-blush border-t-2 border-ink flex flex-col sm:flex-row items-center justify-between gap-4">
                <p class="text-sm text-slate-600">
                  ...
                  <span class="font-semibold text-ink"><?= count($designations) ?></span> of
                  <span class="font-semibold text-ink"><?= count($designations) ?></span> entries
                </p>
  ...
  <div id="designationModal" class="hidden fixed inset-0 z-[60] flex items-center justify-center p-4">
    <div class="absolute inset-0 bg-black/50 backdrop-blur-sm" onclick="closeDesignationModal()"></div>
    <div class="relative w-full max-w-lg bg-white rounded-2xl border-2 border-ink shadow-[8px_8px_0_#FFC93C] animate-modal-in overflow-hidden">
      ...
      <div class="flex items-center justify-between px-6 py-4 border-b-2 border-ink bg-blush">
        <h3 id="designationModalTitle" class="font-display text-lg font-bold text-ink">Add Designation</h3>
        <button type="button" onclick="closeDesignationModal()" aria-label="Close"
      ...
      <form id="designationForm" method="POST" action="designations.php" class="p-6 space-y-5">
        ...
        <div class="space-y-2">
          <label for="designation_name" class="block text-sm font-semibold text-ink">Designation Name <span class="text-hot">*</span></label>
        ...
        <div class="space-y-2">
          <label for="post_occupied" class="block text-sm font-semibold text-ink">Post Occupied <span class="text-hot">*</span></label>
  ...
  <div id="viewModal" class="hidden fixed inset-0 z-[60] flex items-center justify-center p-4">
    ...
    <div class="relative w-full max-w-md bg-white rounded-2xl border-2 border-ink shadow-[8px_8px_0_#FFC93C] animate-modal-in overflow-hidden">
      <div class="h-1.5 w-full bg-hot"></div>
      <div class="flex items-center justify-between px-6 py-4 border-b-2 border-ink bg-blush">
        <h3 class="font-display text-lg font-bold text-ink">Designation Details</h3>
      ...
      <div class="p-6 space-y-4">
        <div class="flex items-center gap-4 pb-4 border-b-2 border-slate-100">
          ...
          <div class="min-w-0">
            <p id="viewDesignationName" class="text-base font-bold text-ink break-words">—</p>
        ...
        <div class="space-y-3">
          ...
          <div class="flex items-start gap-3">
            <i data-lucide="badge-check" class="w-4 h-4 text-hot mt-1 flex-shrink-0"></i>
            <div class="min-w-0">
  ...
  <div id="deleteConfirmModal" class="hidden fixed inset-0 z-[70] flex items-center justify-center p-4">
    ...
    <div id="deleteConfirmPanel" class="relative w-full max-w-md bg-white rounded-2xl border-2 border-ink shadow-[8px_8px_0_#FFC93C] animate-modal-in overflow-hidden">
      ...
      <div class="px-6 pt-6 pb-2 flex flex-col items-center text-center">
        ...
        <h3 class="font-display text-xl font-bold text-ink mb-2">Delete Designation?</h3>
        <p class="text-sm text-slate-500 leading-relaxed">
          You are about to permanently delete
          <span id="deleteDesignationNameDisplay" class="font-bold text-hot break-words">this designation</span>.
        </p>
  ...
  <div id="toggleConfirmModal" class="hidden fixed inset-0 z-[70] flex items-center justify-center p-4">
    ...
    <div id="toggleConfirmPanel" class="relative w-full max-w-md bg-white rounded-2xl border-2 border-ink shadow-[8px_8px_0_#FFC93C] animate-modal-in overflow-hidden">
      <div class="h-1.5 w-full bg-sun"></div>
      <div class="px-6 pt-6 pb-2 flex flex-col items-center text-center">
        ...
        <p id="toggleModalDescription" class="text-sm text-slate-500 leading-relaxed">
          The status of <span class="font-bold text-hot break-words">this designation</span> will be changed.
        </p>
      </div>
      <div class="px-6 py-5 mt-2 flex flex-col-reverse sm:flex-row gap-3">
  ...
  <script>
    ...
    document.getElementById('designationModal').addEventListener('click', function (e) { if (e.target.id === 'designationModal') closeDesignationModal(); });
    document.addEventListener('keydown', function (e) { if (e.key === 'Escape' && !designationModal.classList.contains('hidden')) closeDesignationModal(); });
    ...
    function confirmToggleStatus(designationId, designationName, currentStatus) {
      ...
      if (isCurrentlyActive) {
        ...
        if (toggleModalDescription) toggleModalDescription.innerHTML = 'The designation <span class="font-bold text-hot break-words">"' + designationName + '"</span> will be marked as <span class="font-bold text-hot">Inactive</span>.';
        ...
        if (toggleModalDescription) toggleModalDescription.innerHTML = 'The designation <span class="font-bold text-hot break-words">"' + designationName + '"</span> will be marked as <span class="font-bold text-hot">Active</span>.';
