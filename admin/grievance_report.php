<?php
/**
 * admin/grievance_reports.php
 * ---------------------------------------------------------------------------
 * Admin — Grievance Reports Hub
...
if ($conn instanceof mysqli) {
    try {
        ...
        error_log('[Grievance Reports Admin Profile] ' . $ex->getMessage());
    }
}
...
$reportCards = [
    ['title' => 'Complaint Report',      'icon' => 'file-text',      'href' => 'complaint_report.php'],
    ['title' => 'Cell Members Report',   'icon' => 'users',          'href' => 'cell_members_report.php'],
    ['title' => 'Consolidated Report',   'icon' => 'clipboard-list', 'href' => 'consolidated_report.php'],
    ['title' => 'Summary Report',        'icon' => 'monitor-check',  'href' => 'summary_report.php'],
];
?>
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <meta name="description" content="Grievance reports — Rajagiri College Grievance Redressal Portal." />
  <meta name="theme-color" content="#DB0878" />
  <title>Grievance Reports — Admin | Rajagiri College Grievance Portal</title>
  ...
  <div class="flex min-h-screen">
    ...
    <div class="flex-1 ml-20 flex flex-col min-h-screen">
      ...
      <main id="main-content" class="flex-1 px-4 sm:px-6 py-8">
        ...
        <div class="max-w-6xl mx-auto mb-12 rise text-center">
          <h1 class="font-display balance text-2xl md:text-3xl lg:text-4xl font-extrabold tracking-tight text-ink mb-4">
            Rajagiri College of Social Sciences
          </h1>

          <nav class="flex items-center justify-center gap-2 text-sm text-slate-500" aria-label="Breadcrumb">
            ...
            <span class="text-hot font-semibold">Grievance Reports</span>
          </nav>
        </div>
        ...
        <div class="max-w-5xl mx-auto rise rise-2">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
            ...
            <?php foreach ($reportCards as $index => $card): ?>
              <a href="<?= e($card['href']) ?>"
                 class="card-hard group relative overflow-hidden rounded-2xl bg-white p-6 md:p-7 block">

                <span class="absolute top-0 right-0 w-24 h-24 rounded-full bg-blush -mr-8 -mt-8 group-hover:scale-125 transition-transform duration-500" aria-hidden="true"></span>
                <span class="absolute bottom-0 left-0 w-16 h-16 rounded-full bg-sun/30 -ml-6 -mb-6 group-hover:scale-125 transition-transform duration-500 delay-75" aria-hidden="true"></span>

                <div class="relative flex items-center gap-5">
                  <span class="relative flex-shrink-0 inline-flex items-center justify-center w-20 h-20 rounded-2xl bg-hot border-2 border-ink shadow-[3px_3px_0_#FFC93C] group-hover:scale-110 group-hover:rotate-6 transition-transform duration-300">
                    <i data-lucide="<?= e($card['icon']) ?>" class="w-9 h-9 text-white"></i>
                  </span>

                  <div class="min-w-0 flex-1">
                    <h3 class="font-display text-lg md:text-xl font-bold text-ink mb-1 group-hover:text-hot transition-colors">
                      <?= e($card['title']) ?>
                    </h3>
                    <p class="text-sm font-bold text-hot group-hover:text-hotdark transition-colors inline-flex items-center gap-1.5">
                ...
                <span class="absolute bottom-0 left-0 right-0 h-1.5 bg-hot rounded-b-2xl transform scale-x-0 group-hover:scale-x-100 transition-transform duration-500 origin-left"></span>
              </a>
            <?php endforeach; ?>
          </div>
        </div>
