...
$adminData = ['username' => $_SESSION['username'] ?? 'Admin', 'name' => '', 'email' => '', 'profile_picture' => ''];
...
if ($conn instanceof mysqli) {
    ...
    } catch (Throwable $ex) { error_log('[Complaint Report Admin Profile] ' . $ex->getMessage()); }
}
...
$grievanceTypes = []; $cellMembers = []; $courses = []; $classes = []; $departments = []; $designations = [];

if ($conn instanceof mysqli) {
    try { $res = $conn->query("SELECT id, type_name FROM grievance_types WHERE status = 'Active' ORDER BY type_name ASC"); if ($res) while ($row = $res->fetch_assoc()) $grievanceTypes[] = $row; } catch (Throwable $ex) { error_log('[Fetch Grievance Types] ' . $ex->getMessage()); }
    try { $res = $conn->query("SELECT id, name FROM cell_members ORDER BY name ASC"); if ($res) while ($row = $res->fetch_assoc()) $cellMembers[] = $row; } catch (Throwable $ex) { error_log('[Fetch Cell Members] ' . $ex->getMessage()); }
    try { $res = $conn->query("SELECT id, course_name FROM courses WHERE status = 'Active' ORDER BY course_name ASC"); if ($res) while ($row = $res->fetch_assoc()) $courses[] = $row; } catch (Throwable $ex) { error_log('[Fetch Courses] ' . $ex->getMessage()); }
    try { $res = $conn->query("SELECT id, class_name FROM classes WHERE status = 'Active' ORDER BY class_name ASC"); if ($res) while ($row = $res->fetch_assoc()) $classes[] = $row; } catch (Throwable $ex) { error_log('[Fetch Classes] ' . $ex->getMessage()); }
    try { $res = $conn->query("SELECT id, department_name FROM departments WHERE status = 'Active' ORDER BY department_name ASC"); if ($res) while ($row = $res->fetch_assoc()) $departments[] = $row; } catch (Throwable $ex) { error_log('[Fetch Departments] ' . $ex->getMessage()); }
    try { $res = $conn->query("SELECT id, designation_name FROM designations WHERE status = 'Active' ORDER BY designation_name ASC"); if ($res) while ($row = $res->fetch_assoc()) $designations[] = $row; } catch (Throwable $ex) { error_log('[Fetch Designations] ' . $ex->getMessage()); }
}
...
if (strtotime($filterFrom) > strtotime($filterTo)) { [$filterFrom, $filterTo] = [$filterTo, $filterFrom]; }
...
if ($isSubmitted && $conn instanceof mysqli) {
    try {
        ...
        if ($filterType > 0)        { $sql .= " AND g.grievance_type_id = ?"; $params[] = $filterType;        $types .= 'i'; }
        if ($filterMember > 0)      { $sql .= " AND g.assigned_member_id = ?"; $params[] = $filterMember;      $types .= 'i'; }
        if ($filterCourse > 0)      { $sql .= " AND s.class_id IN (SELECT id FROM classes WHERE course_id = ?)"; $params[] = $filterCourse; $types .= 'i'; }
        if ($filterClass > 0)       { $sql .= " AND s.class_id = ?";         $params[] = $filterClass;          $types .= 'i'; }
        if ($filterDepartment > 0)  { $sql .= " AND cm.department_id = ?";   $params[] = $filterDepartment;     $types .= 'i'; }
        if ($filterDesignation > 0) { $sql .= " AND cm.designation_id = ?";  $params[] = $filterDesignation;    $types .= 'i'; }
        if ($filterStatus !== '')   { $sql .= " AND g.status = ?";           $params[] = $filterStatus;         $types .= 's'; }
    ...
    } catch (Throwable $ex) { error_log('[Complaint Report] ' . $ex->getMessage()); }
}

function findName(array $list, int $id, string $key): string
{
    foreach ($list as $item) if ((int) $item['id'] === $id) return (string) $item[$key];
    return 'All';
}
?>
<!DOCTYPE html>
...
<head>
  ...
  <meta name="description" content="Complaint report — Rajagiri College Grievance Redressal Portal." />
  <meta name="theme-color" content="#DB0878" />
  <title>Complaint Report — RCSS Admin</title>
  ...
  <style>
    ...
    @media print {
      ...
      .report-table th, .report-table td { border: 1px solid #333; padding: 5px 6px; text-align: left; }
      ...
      <main id="main-content" class="flex-1 px-4 sm:px-6 py-8">
        ...
        <div class="max-w-6xl mx-auto mb-6 rise">
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <h1 class="font-display text-2xl md:text-3xl font-extrabold tracking-tight text-ink mb-2">Complaint Report</h1>
              <nav class="flex flex-wrap items-center gap-2 text-sm text-slate-500" aria-label="Breadcrumb">
                ...
                <span class="text-slate-300">/</span>
                <span class="text-hot font-semibold">Complaint Report</span>
              </nav>
            </div>
          ...
          <div class="bg-white rounded-2xl border-2 border-ink shadow-[6px_6px_0_#FFC93C] overflow-hidden">
            <div class="flex items-center justify-between px-6 py-4 border-b-2 border-ink bg-blush">
              <div class="flex items-center gap-3">
                ...
                <h2 class="font-display font-bold text-ink">Build your report</h2>
              </div>
              <span class="text-xs font-semibold text-slate-500">Results appear after submission</span>
            </div>

            <div class="p-6">
              <form method="GET" action="complaint_report.php">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                  ...
                  <div class="space-y-2">
                    <label for="grievance_member_id" class="block text-sm font-semibold text-ink">Grievance member</label>
                    <select name="grievance_member_id" id="grievance_member_id"
                            class="w-full px-4 py-3 border-2 border-ink rounded-xl bg-white text-ink font-medium focus:outline-none focus:ring-4 focus:ring-hot/10 transition-all">
                      <option value="0">All members</option>
                      <?php foreach ($cellMembers as $cm): ?>
                        <option value="<?= (int) $cm['id'] ?>" <?= $filterMember === (int) $cm['id'] ? 'selected' : '' ?>><?= e($cm['name']) ?></option>
                      <?php endforeach; ?>
                    </select>
                  </div>

                  <div class="space-y-2">
                    <label for="course_id" class="block text-sm font-semibold text-ink">Course</label>
                ...
                <div class="mt-5 pt-5 border-t-2 border-dashed border-slate-200 flex flex-col sm:flex-row gap-3 sm:justify-end">
                  ...
                  <button type="submit"
                    ...
                    <span>Run complaint report</span>
                  </button>
                </div>
              </form>
            </div>
            ...
            <section class="bg-white rounded-2xl border-2 border-ink shadow-[6px_6px_0_#FFC93C] overflow-hidden" id="reportSection">
              <div class="p-6 border-b-2 border-ink bg-white flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div class="flex items-center gap-4">
                  ...
                  <div>
                    <h3 class="font-display font-bold text-ink text-lg">Complaint Report</h3>
                    <p class="text-xs text-slate-500">Filtered grievance records · <?= e(date('d/m/Y', strtotime($filterFrom))) ?> – <?= e(date('d/m/Y', strtotime($filterTo))) ?></p>
                  </div>
                </div>
                <div class="text-right text-xs text-slate-500">
                  <strong class="text-ink text-lg font-display"><?= count($rows) ?></strong><br>
                  matching record(s)
                </div>
              </div>
                ...
                <table class="w-full report-table">
                  <thead>
                    <tr class="bg-ink text-white">
                      ...
                      <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider">Reply</th>
                      <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider">Feedback</th>
                    </tr>
                  </thead>
                  <tbody class="divide-y divide-slate-100">
                        ...
                        <td colspan="10" class="px-6 py-16 text-center text-slate-500">
                          <div class="flex flex-col items-center justify-center">
                            <span class="w-16 h-16 rounded-full bg-blush border-2 border-ink flex items-center justify-center mb-4">
                              <i data-lucide="file-x-2" class="w-8 h-8 text-hot"></i>
                            </span>
                            <p class="text-lg font-bold text-ink">No grievances match the selected filters.</p>
                          </div>
                        </td>
                      </tr>
                    <?php else: foreach ($rows as $i => $r):
                      ...
                      $descPreview = mb_strlen($desc, 'UTF-8') > 120 ? mb_substr($desc, 0, 120, 'UTF-8') . '…' : $desc;
                      $reply = (string) ($r['reply_details'] ?? '');
                      $replyPreview = mb_strlen($reply, 'UTF-8') > 120 ? mb_substr($reply, 0, 120, 'UTF-8') . '…' : $reply;
                      $fb    = (string) ($r['feedback_details'] ?? '');
                      $fbPreview = mb_strlen($fb, 'UTF-8') > 120 ? mb_substr($fb, 0, 120, 'UTF-8') . '…' : $fb;
                      $sl    = strtolower((string) ($r['status'] ?? ''));
                      $statusClass = $sl === 'pending'
                          ? 'bg-[#FFF5D8] text-[#9A6500] border-[#F1DDA1]'
                          : (($sl === 'closed' || $sl === 'disposed')
                    ...
                    ?>
                      <tr class="hover:bg-blush/60 transition-colors">
                        ...
                        <td class="px-4 py-3 text-sm">
                          <p class="font-bold text-ink"><?= e($r['complainant_name'] ?? '—') ?></p>
                          <p class="text-xs text-slate-500"><?= e($r['complainant_email'] ?? '') ?></p>
                        </td>
                        <td class="px-4 py-3 text-sm text-slate-700 whitespace-nowrap"><?= e($rDate) ?></td>
                        <td class="px-4 py-3 text-sm text-slate-700"><?= e($r['type_name'] ?? '—') ?></td>
                        <td class="px-4 py-3 text-sm font-semibold text-ink"><?= e($r['subject'] ?? '—') ?></td>
                        <td class="px-4 py-3 text-sm text-slate-600" title="<?= e($desc) ?>"><?= e($descPreview ?: '—') ?></td>
                        <td class="px-4 py-3 whitespace-nowrap">
                          <span class="inline-flex items-center px-3 py-1 text-xs font-bold rounded-full border-2 <?= $statusClass ?>"><?= e($r['status'] ?? '—') ?></span>
                        </td>
                        <td class="px-4 py-3 text-sm text-slate-700"><?= e($r['attended_by_name'] ?? '—') ?></td>
                        <td class="px-4 py-3 text-sm text-slate-600" title="<?= e($reply) ?>"><?= e($replyPreview ?: '—') ?></td>
                        <td class="px-4 py-3 text-sm text-slate-600" title="<?= e($fb) ?>"><?= e($fbPreview ?: '—') ?></td>
                      </tr>
              ...
              <div class="px-6 py-4 border-t-2 border-ink bg-blush text-xs text-slate-600">
                Generated <?= date('d-m-Y H:i') ?> · <?= count($rows) ?> result(s)
              </div>
            </section>
          </div>
        <?php endif; ?>
