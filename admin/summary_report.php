...
<head>
  ...
  <meta name="description" content="Summary report — Rajagiri College Grievance Redressal Portal." />
  <meta name="theme-color" content="#DB0878" />
  <title>Summary Report — RCSS Admin</title>
      ...
      <main id="main-content" class="flex-1 px-4 sm:px-6 py-8">
        ...
        <div class="max-w-6xl mx-auto mb-6 rise">
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <h1 class="font-display text-2xl md:text-3xl font-extrabold tracking-tight text-ink mb-2">Summary Report</h1>
              <nav class="flex flex-wrap items-center gap-2 text-sm text-slate-500" aria-label="Breadcrumb">
                ...
                <span class="text-slate-300">/</span>
                <span class="text-hot font-semibold">Summary Report</span>
              </nav>
            </div>
        ...
        <div class="max-w-6xl mx-auto mb-6 rise rise-2 no-print">
          <div class="bg-white rounded-2xl border-2 border-ink shadow-[6px_6px_0_#FFC93C] overflow-hidden">
            ...
            <div class="p-6">
              <form method="GET" action="summary_report.php">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-5 items-end">
                  ...
                  <div>
                    <button type="submit"
                            class="btn-hard w-full inline-flex items-center justify-center gap-2 rounded-xl bg-hot px-8 py-3 text-sm font-bold text-white hover:bg-hotdark">
        ...
        <!-- NO RECORDS ALERT -->
        <?php if ($reportSubmitted && empty($summaryRows)): ?>
          <div class="max-w-6xl mx-auto mb-6 rise rise-3">
            <div class="rounded-2xl bg-hot text-white px-5 py-4 flex items-center gap-3 border-2 border-ink shadow-[6px_6px_0_#FFC93C]">
              <i data-lucide="alert-circle" class="w-5 h-5 flex-shrink-0"></i>
              <p class="text-sm font-bold tracking-wide">no records found !!!</p>
            </div>
          </div>
        <?php endif; ?>
        ...
        <!-- REPORT -->
        <?php if ($reportSubmitted && !empty($summaryRows)): ?>
          <div id="reportSection" class="max-w-6xl mx-auto rise rise-3">
            <section class="bg-white rounded-2xl border-2 border-ink shadow-[6px_6px_0_#FFC93C] overflow-hidden">
              ...
              <div class="p-6 border-b-2 border-ink bg-white flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div class="flex items-center gap-4">
                  ...
                  <div>
                    <h3 class="font-display font-bold text-ink text-lg">Summary Report</h3>
                    <p class="text-xs text-slate-500">Rajagiri College of Social Sciences · Grievance Redressal Portal</p>
                  </div>
                </div>
                <div class="text-right text-xs text-slate-500">
                  <strong class="text-ink text-sm">Date: <?= date('d-m-Y') ?></strong><br>
                  Total Categories: <span class="font-bold text-ink"><?= count($summaryRows) ?></span>
                </div>
              </div>
                  ...
                  <tbody class="divide-y divide-slate-100 bg-white">
                    <?php foreach ($summaryRows as $i => $r): ?>
                      <tr class="hover:bg-blush/60 transition-colors">
                        ...
                        <td class="px-4 py-3 text-sm font-semibold text-ink"><?= e($r['type_name'] ?? '—') ?></td>
                        <td class="px-4 py-3 text-sm text-right font-bold text-ink"><?= (int) $r['total'] ?></td>
                        <td class="px-4 py-3 text-sm text-right text-slate-700"><?= (int) $r['pending'] ?></td>
                        <td class="px-4 py-3 text-sm text-right text-slate-700"><?= (int) $r['in_progress'] ?></td>
                        <td class="px-4 py-3 text-sm text-right text-slate-700"><?= (int) $r['closed'] ?></td>
                      </tr>
                    <?php endforeach; ?>
                  </tbody>

                  <tfoot>
                    <tr class="bg-sun border-t-2 border-ink">
                      <td class="px-4 py-3 text-sm font-extrabold text-ink text-right" colspan="2">Grand Total</td>
                      <td class="px-4 py-3 text-sm font-extrabold text-ink text-right"><?= (int) $grandTotals['total'] ?></td>
                      <td class="px-4 py-3 text-sm font-extrabold text-ink text-right"><?= (int) $grandTotals['pending'] ?></td>
                      <td class="px-4 py-3 text-sm font-extrabold text-ink text-right"><?= (int) $grandTotals['in_progress'] ?></td>
                      <td class="px-4 py-3 text-sm font-extrabold text-ink text-right"><?= (int) $grandTotals['closed'] ?></td>
                    </tr>
                  </tfoot>
                </table>
              </div>
              ...
              <div class="px-6 py-4 border-t-2 border-ink bg-blush text-xs text-slate-600">
                Generated on <?= date('d-m-Y H:i') ?> · Period <?= e(date('d/m/Y', strtotime($filterFrom))) ?> – <?= e(date('d/m/Y', strtotime($filterTo))) ?>
              </div>
            </section>
          </div>
        <?php endif; ?>
