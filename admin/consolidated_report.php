<?php
/**
 * admin/consolidated_report.php
 * ---------------------------------------------------------------------------
 * Admin — Consolidated Report (Aggregated Grievance Statistics)
 * Rajagiri College Grievance Redressal Portal
 *
 * Features:
 *   • Filter by date range (From / To)
 *   • Aggregated counts: Pending, Closed/Disposed, Total
 *   • Report shown ONLY after Submit (hidden by default)
...
if ($conn instanceof mysqli) {
    try {
        ...
        error_log('[Consolidated Report Admin Profile] ' . $ex->getMessage());
    }
}
...
if ($reportSubmitted && $conn instanceof mysqli) {
    try {
        $sql = "SELECT COUNT(*) AS c FROM grievances WHERE status = 'Pending' AND DATE(created_at) BETWEEN ? AND ?";
        $stmt = $conn->prepare($sql);
        if ($stmt) {
            $stmt->bind_param('ss', $filterFrom, $filterTo);
            $stmt->execute();
            $res = $stmt->get_result();
            $pendingCount = (int) ($res ? ($res->fetch_assoc()['c'] ?? 0) : 0);
            $stmt->close();
        }

        $sql = "SELECT COUNT(*) AS c FROM grievances WHERE status IN ('Closed', 'Disposed') AND DATE(created_at) BETWEEN ? AND ?";
        $stmt = $conn->prepare($sql);
        if ($stmt) {
            $stmt->bind_param('ss', $filterFrom, $filterTo);
            $stmt->execute();
            $res = $stmt->get_result();
            $closedCount = (int) ($res ? ($res->fetch_assoc()['c'] ?? 0) : 0);
            $stmt->close();
        }

        $sql = "SELECT COUNT(*) AS c FROM grievances WHERE DATE(created_at) BETWEEN ? AND ?";
        $stmt = $conn->prepare($sql);
        if ($stmt) {
            $stmt->bind_param('ss', $filterFrom, $filterTo);
            $stmt->execute();
            $res = $stmt->get_result();
            $totalCount = (int) ($res ? ($res->fetch_assoc()['c'] ?? 0) : 0);
            $stmt->close();
        }
    } catch (Throwable $ex) {
        error_log('[Consolidated Report] ' . $ex->getMessage());
    }
}

$reportRows = [
    ['label' => 'Pending Grievances', 'count' => $pendingCount],
    ['label' => 'Closed Grievances',  'count' => $closedCount],
    ['label' => 'Total Grievances',   'count' => $totalCount],
];
?>
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <meta name="description" content="Consolidated report — Rajagiri College Grievance Redressal Portal." />
  <meta name="theme-color" content="#DB0878" />
  <title>Consolidated Report — RCSS Admin</title>
  ...
  <style>
    ...
    .metric-hard:nth-child(1) { box-shadow: 5px 5px 0 #DB0878; }
    .metric-hard:nth-child(2) { box-shadow: 5px 5px 0 #FFC93C; }
    .metric-hard:nth-child(3) { box-shadow: 5px 5px 0 #006837; }
    ...
    @media print {
      ...
      .report-table th, .report-table td {
        border: 1px solid #333; padding: 6px 8px; text-align: left;
      }
      .report-table th {
      ...
      <main id="main-content" class="flex-1 px-4 sm:px-6 py-8">
        ...
        <div class="max-w-6xl mx-auto mb-6 rise">
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <h1 class="font-display text-2xl md:text-3xl font-extrabold tracking-tight text-ink mb-2">Consolidated Report</h1>
              <nav class="flex flex-wrap items-center gap-2 text-sm text-slate-500" aria-label="Breadcrumb">
                ...
                <span class="text-slate-300">/</span>
                <span class="text-hot font-semibold">Consolidated Report</span>
              </nav>
            </div>
          ...
          <div class="bg-white rounded-2xl border-2 border-ink shadow-[6px_6px_0_#FFC93C] overflow-hidden">
            <div class="flex items-center justify-between px-6 py-4 border-b-2 border-ink bg-blush">
              <div class="flex items-center gap-3">
                ...
                <h2 class="font-display font-bold text-ink">Select reporting period</h2>
              </div>
              <span class="text-xs font-semibold text-slate-500">30-day view by default</span>
            </div>
            <div class="p-6">
              <form method="GET" action="consolidated_report.php">
                ...
                <div class="mt-5 pt-5 border-t-2 border-dashed border-slate-200 flex justify-end">
                  <button type="submit" class="btn-hard inline-flex items-center gap-2 rounded-xl bg-hot px-8 py-3 text-sm font-bold text-white hover:bg-hotdark">
                    <i data-lucide="refresh-cw" class="w-4 h-4"></i> Generate overview
                  </button>
                </div>
              </form>
            </div>
          ...
          <div class="max-w-6xl mx-auto rise rise-3">
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-8">
              <?php foreach ($reportRows as $row): ?>
                <div class="metric-hard rounded-2xl bg-white p-6">
                  <p class="text-xs font-bold text-hot uppercase tracking-wider"><?= e($row['label']) ?></p>
                  <p class="mt-2 font-display font-extrabold text-ink text-4xl md:text-5xl leading-none"><?= (int) $row['count'] ?></p>
                </div>
              <?php endforeach; ?>
            </div>

            <section class="panel-hard bg-white rounded-2xl overflow-hidden" id="reportSection">
              <div class="p-6 border-b-2 border-ink bg-white flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div class="flex items-center gap-4">
                  ...
                  <div>
                    <h3 class="font-display font-bold text-ink text-lg">Consolidated Report</h3>
                    <p class="text-xs text-slate-500">Rajagiri College of Social Sciences · Grievance Redressal Portal</p>
                  </div>
                </div>
                <div class="text-right text-xs text-slate-500">
                  Period<br>
                  <strong class="text-ink text-sm"><?= e(date('d/m/Y', strtotime($filterFrom))) ?> – <?= e(date('d/m/Y', strtotime($filterTo))) ?></strong>
                </div>
              </div>
                ...
                <table class="w-full report-table">
                  <thead>
                    <tr class="bg-ink text-white">
                      <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider" style="width: 90px;">Sl No</th>
                      <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider">Grievance</th>
                      <th class="px-4 py-3 text-right text-xs font-bold uppercase tracking-wider">Number</th>
                    </tr>
                  </thead>
                  <tbody class="divide-y divide-slate-100">
                    <?php foreach ($reportRows as $i => $row): ?>
                      <tr class="hover:bg-blush/60 transition-colors">
                        ...
                        <td class="px-4 py-3 text-sm font-semibold text-ink"><?= e($row['label']) ?></td>
                        <td class="px-4 py-3 text-right font-display font-extrabold text-ink text-xl"><?= (int) $row['count'] ?></td>
                      </tr>
                    <?php endforeach; ?>
                  </tbody>
                </table>
