...
<head>
  ...
  <meta name="description" content="Edit admin profile — Rajagiri College Grievance Redressal Portal." />
  ...
  <div class="flex min-h-screen">
    ...
    <aside class="w-20 on-ink bg-ink flex flex-col items-center py-4 fixed inset-y-0 left-0 z-40">
      ...
      <nav class="flex flex-col items-center space-y-4 flex-1 mt-8" aria-label="Admin navigation">
        <a href="profile.php" class="nav-icon-link group relative w-12 h-12 rounded-xl bg-hot flex items-center justify-center text-white ring-2 ring-sun/60" title="Profile" aria-label="Profile">
        ...
        <div class="flex items-center justify-between px-4 sm:px-6 py-3 md:py-4">
          ...
          <div class="flex items-center gap-3">
            <?php if ($existingPreviewUrl): ?>
              <img src="<?= e($existingPreviewUrl) ?>" alt="Admin" class="w-10 h-10 rounded-full object-cover border-2 border-ink shadow-[2px_2px_0_#FFC93C]" />
            <?php else: ?>
            ...
            <?php endif; ?>
            <span class="hidden sm:block text-sm font-bold text-ink"><?= e($formData['username'] ?: 'Admin') ?></span>
          </div>
        </div>
      </header>
            ...
            <form action="edit_profile.php" method="POST" enctype="multipart/form-data" class="p-6 md:p-8 space-y-8">
              ...
              <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                  ...
                  <div class="space-y-2">
                    <label class="block text-sm font-semibold text-ink">Mobile Number <span class="text-hot">*</span></label>
                    <div class="flex gap-2">
                      <div class="relative w-44 flex-shrink-0" data-country-select="mobile">
                        ...
                        <button type="button" class="country-select-trigger w-full flex items-center justify-between px-3 py-3 border-2 border-ink rounded-xl bg-white text-ink text-sm font-medium focus:outline-none focus:ring-4 focus:ring-hot/10 transition-all">
                        ...
                        <div class="country-select-panel hidden absolute z-50 mt-1 w-72 bg-white rounded-xl border-2 border-ink shadow-[6px_6px_0_#FFC93C] overflow-hidden">
                          ...
                          <div class="country-select-list max-h-56 overflow-y-auto">
                            <?php foreach ($countryCodes as $code => $label): ?>
                              <button type="button"
                                      class="country-select-option w-full flex items-center justify-between px-4 py-2.5 text-sm transition-colors <?= $formData['mobile_country_code'] === $code ? 'bg-blush text-hot font-semibold' : 'text-slate-700 hover:bg-blush' ?>"
                  ...
                  <div class="space-y-2">
                    <label class="block text-sm font-semibold text-ink">WhatsApp Number</label>
                    <div class="flex gap-2">
                      <div class="relative w-44 flex-shrink-0" data-country-select="whatsapp">
                        ...
                        <div class="country-select-panel hidden absolute z-50 mt-1 w-72 bg-white rounded-xl border-2 border-ink shadow-[6px_6px_0_#FFC93C] overflow-hidden">
                          ...
                          <div class="country-select-list max-h-56 overflow-y-auto">
                            <button type="button" class="country-select-option w-full flex items-center justify-between px-4 py-2.5 text-sm transition-colors <?= $formData['whatsapp_country_code'] === '' ? 'bg-blush text-hot font-semibold' : 'text-slate-700 hover:bg-blush' ?>" data-value="" data-label="--SELECT--" data-search="select">
                            ...
                            <?php foreach ($countryCodes as $code => $label): ?>
                              <button type="button"
                                      class="country-select-option w-full flex items-center justify-between px-4 py-2.5 text-sm transition-colors <?= $formData['whatsapp_country_code'] === $code ? 'bg-blush text-hot font-semibold' : 'text-slate-700 hover:bg-blush' ?>"
                                      data-value="<?= e($code) ?>" data-label="<?= e($label) ?>" data-search="<?= e(strtolower($code . ' ' . $label)) ?>">
                                <span class="font-semibold flex-shrink-0"><?= e($code) ?></span>
                                <span class="text-xs text-slate-500 flex-1 ml-3 text-left truncate"><?= e($label) ?></span>
                              </button>
        ...
        options.forEach(function (opt) {
          opt.addEventListener('click', function (e) {
            ...
            options.forEach(function (o) {
              o.classList.remove('bg-blush', 'text-hot', 'font-semibold');
            });
